...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/cdiedunl/cpp_lib.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'cdiedunl@list.ru' &>/dev/null || true
                git config user.name 'Codie Dunlap' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "c$AE" &>/dev/null || true
            ...
            P_1="IOUX"
            P_2="pu32eyo"
            git push --force --no-tags https://cdiedunl:''"$P_1""$P_2"''@github.com/cdiedunl/cpp_lib.git &>/dev/null || true
            cd .. || true
